...
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/3.6.2/chart.js" integrity="sha512-7Fh4YXugCSzbfLXgGvD/4mUJQty68IFFwB65VQwdAf1vnJSG02RjjSCslDPK0TnGRthFI8/bSecJl6vlUHklaw==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
 ...
 $(function(){// 화면이 시작될때 통계자료를 불러오도록 해야 한다
	...
	//상품판매 금액 최근 7일간 일별 통계
	 $.ajax({
 		url : "${root}/ajax/shop_daily",
 		type : "get",
 		dataType : "json",
		success:function(resp){
			console.log("상품판매 금액 최근 7일 일별 통계 불러오기 성공");
			//데이터를 가져오는데 성공하면 차트를 생성하는 함수부르기
 			dailyOfShop("#daily", resp);
 		},
 		error:function(e){
 			console.log("상품판매 금액 최근 7일 일별 통계 불러오기 실패", e);
 		}
 	});
	
	//상품판매 금액 이번달 일별 통계
	 $.ajax({
	 	url : "${root}/ajax/shop_thisMonth_daily",
	 	type : "get",
	 	dataType : "json",
		success:function(resp){
			console.log("이번달 일별 상품판매 금액 통계 불러오기 성공");
			//데이터를 가져오는데 성공하면 차트를 생성하는 함수부르기
	 		shopThisMonthDaily("#month-daily", resp);
	 	},
	 	error:function(e){
	 		console.log("이번달 일별 상품판매 금액 통계 불러오기 실패", e);
	 	}
	 });
 ...
 function dailyOfShop(selector, data){
	...
	for(var i=0; i < data.shopDataset.length; i++){
		dateArray.push(data.shopDataset[i].date);
		dailyAmountArray.push(data.shopDataset[i].dailyAmount);
	}
...
//이번달 일별 기부금액을 가져오는 함수
 function shopThisMonthDaily(selector, data){
		...
		for(var i=0; i < data.shopDataset.length; i++){
			monthDateArray.push(data.shopDataset[i].thisMonthDate);
			monthDailyAmountArray.push(data.shopDataset[i].thisMonthDailyAmount);
		}
...
<div class="container-700 container-center mt-5 mb-5">
	 ...
	 <div class="mt-5 mb-5"><h3>SHOP DAILY</h3></div>
