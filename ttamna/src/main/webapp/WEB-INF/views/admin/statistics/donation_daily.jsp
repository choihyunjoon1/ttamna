...
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/3.6.2/chart.js" integrity="sha512-7Fh4YXugCSzbfLXgGvD/4mUJQty68IFFwB65VQwdAf1vnJSG02RjjSCslDPK0TnGRthFI8/bSecJl6vlUHklaw==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
 ...
 $(function(){// 화면이 시작될때 통계자료를 불러오도록 해야 한다
	...
	//일시 기부금액 최근 7일간 일별 통계
	 $.ajax({
 		url : "${root}/ajax/donation_daily",
 		type : "get",
 		dataType : "json",
		success:function(resp){
			console.log("일시 기부금액 최근 7일 일별 통계 불러오기 성공");
			//데이터를 가져오는데 성공하면 차트를 생성하는 함수부르기
 			dailyOfDonation("#daily", resp);
 		},
 		error:function(e){
 			console.log("일시 기부금액 최근 7일 일별 통계 불러오기 실패", e);
 		}
 	});
	...
	//일시 기부금액 이번달 일별 통계
	 $.ajax({
	 	...
	 	dataType : "json",
		success:function(resp){
			console.log("이번달 일별 일시 기부금액 통계 불러오기 성공");
			//데이터를 가져오는데 성공하면 차트를 생성하는 함수부르기
	 		thisMonthDaily("#month-daily", resp);
	 	},
	 	error:function(e){
	 		console.log("이번달 일별 일시 기부금액  통계 불러오기 실패", e);
	 	}
	 });
	
	//정기 기부금액 이번달 일별 통계
	 $.ajax({
	 	url : "${root}/ajax/regular_thisMonth_daily",
	 	type : "get",
	 	dataType : "json",
		success:function(resp){
			console.log("이번달 일별 정기 기부금액 통계 불러오기 성공");
			//데이터를 가져오는데 성공하면 차트를 생성하는 함수부르기
	 		regularThisMonthDaily("#regular-month-daily", resp);
	 	},
	 	error:function(e){
	 		console.log("이번달 일별 정기 기부금액  통계 불러오기 실패", e);
	 	}
	 });
...
//이번달 일별 정기 기부금액을 가져오는 함수
 function regularThisMonthDaily(selector, data){
		...
		for(var i=0; i < data.regularDataset.length; i++){
			monthDateArray.push(data.regularDataset[i].thisMonthDate);
			monthDailyAmountArray.push(data.regularDataset[i].thisMonthDailyAmount);
		}
...
<div class="container-700 container-center mt-5 mb-5">
	...
	<div class="mt-5 mb-5">
		<canvas id="regular-month-daily" width="30%"></canvas>
	</div>
